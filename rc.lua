...
volume = lain.widget.alsabar({
  ticks = true, width = 67,
  notification_preset = { font = font }
})
volume.tooltip.wibox.fg = fg_focus
volume.tooltip.wibox.font = font
volume.bar:buttons(my_table.join (
        awful.button({}, 2, function()  -- middle click: sets the volume to its default
          os.execute(string.format("%s set %s 80%%", volume.cmd, volume.channel))
          volume.update()
        end),
        awful.button({}, 3, function()  -- right click: mutes volume
          os.execute(string.format("%s set %s toggle", volume.cmd, volume.togglechannel or volume.channel))
          volume.update()
        end),
        awful.button({}, 4, function()  -- roll up: increases volume
          os.execute(string.format("%s set %s 5%%+", volume.cmd, volume.channel))
          volume.update()
        end),
        awful.button({}, 5, function()  -- roll down: decreases volume
          os.execute(string.format("%s set %s 5%%-", volume.cmd, volume.channel))
          volume.update()
        end)
))
